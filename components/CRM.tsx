...
import { Plus, DollarSign, Calendar, ArrowRight, CheckCircle2, Briefcase, Code, User, Trash2, Megaphone, Instagram, Linkedin, Video, Image, FileText, Zap } from 'lucide-react';
...
const INITIAL_DEALS: Deal[] = [
    { 
        ...
        problemStatement: 'Losing 40% of leads because they cannot reply fast enough on WhatsApp.',
        notes: 'Client is skeptical about AI sounding robotic. Needs a demo.'
    },
    { 
        id: '2', 
        ...
        problemStatement: 'Sales team wasting time on low quality leads.',
        notes: 'Budget approved, waiting for SOW.'
    },
];

const INITIAL_PROJECTS: Project[] = [
    { id: '1', clientName: 'Urban Shoes', featureSummary: 'Product Desc Generator', deadline: 'Oct 24', status: 'In Progress' },
];

const INITIAL_MARKETING: MarketingTask[] = [
    { id: '1', title: 'AI Trends 2024', contentType: 'Carousel', platform: 'LinkedIn', status: 'Design', dueDate: 'Oct 25' },
    { id: '2', title: 'Bokle Client Win', contentType: 'Reel', platform: 'Instagram', status: 'Scripting', dueDate: 'Oct 28' },
];
...
const CRM: React.FC = () => {
    ...
    const [marketingTasks, setMarketingTasks] = useState<MarketingTask[]>(INITIAL_MARKETING);
    ...
    const handleAdd = (e: React.FormEvent) => {
        ...
        if (view === 'sales') {
            ...
            setMarketingTasks([...marketingTasks, newTask]);
        }
    ...
    const updateMarketingStatus = (id: string, newStatus: MarketingStatus) => {
        setMarketingTasks(marketingTasks.map(t => t.id === id ? { ...t, status: newStatus } : t));
    };

    const deleteItem = (id: string) => {
        if (view === 'sales') setDeals(deals.filter(d => d.id !== id));
        if (view === 'tech') setProjects(projects.filter(p => p.id !== id));
        if (view === 'marketing') setMarketingTasks(marketingTasks.filter(t => t.id !== id));
    };
    ...
    // If Autopilot is active, show that component instead
    if (autopilotDeal) {
        return (
            <LifecycleAutopilot 
                deal={autopilotDeal} 
                ...
                onUpdateStatus={(id, status) => {
                    ...
                    // Update the local autopilot deal state so the UI reflects the change immediately
    ...
    return (
        <div className="space-y-6 h-full flex flex-col">
            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-[#373737]">Bokle CRM & Operations</h2>
                ...
                <div className="flex bg-gray-200 p-1 rounded-lg self-start overflow-x-auto">
                    <button
                        ...
                        className={`px-4 py-2 rounded-md text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${
                            view === 'sales' ? 'bg-white text-[#15621B] shadow-sm' : 'text-gray-600 hover:text-gray-900'
                        }`}
                    >
                        <Briefcase size={16} />
                        Sales
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-md text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${
                            view === 'marketing' ? 'bg-white text-pink-700 shadow-sm' : 'text-gray-600 hover:text-gray-900'
                        }`}
                    >
                        <Megaphone size={16} />
                        Content Calendar
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-md text-sm font-medium flex items-center gap-2 transition-all whitespace-nowrap ${
                            view === 'tech' ? 'bg-white text-[#373737] shadow-sm' : 'text-gray-600 hover:text-gray-900'
                        }`}
                    >
                        <Code size={16} />
                        Tech Delivery
            ...
            {!isAdding ? (
                ...
                >
                    <div className="flex-1 w-full">
                        ...
                        <input 
                            className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-[#15621B] outline-none text-black" 
                            value={input1} onChange={e => setInput1(e.target.value)} required 
                            placeholder={view === 'marketing' ? "e.g. 5 AI Hacks" : "e.g. Acme Corp"} 
                        />
                    </div>
                    ...
                    {view === 'marketing' ? (
                        <>
                            <div className="w-32">
                                ...
                                <select 
                                    className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-[#15621B] outline-none bg-white text-black"
                        ...
                        <div className="flex-1 w-full">
                            <label className="text-xs font-semibold text-gray-500 uppercase">
                                {view === 'sales' ? 'Service' : 'Feature Summary'}
                            </label>
                            <input 
                                className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-[#15621B] outline-none text-black" 
                                value={input3} onChange={e => setInput3(e.target.value)} required 
                                placeholder={view === 'sales' ? "Chatbot" : "Core API"} 
                            />
                        </div>
                    )}
                    ...
                    <div className="w-32">
                        <label className="text-xs font-semibold text-gray-500 uppercase">
                            {view === 'sales' ? 'Value' : 'Deadline'}
                        </label>
                        <input 
                            className="w-full p-2 border border-gray-300 rounded focus:ring-2 focus:ring-[#15621B] outline-none text-black" 
                            value={input2} onChange={e => setInput2(e.target.value)} required 
                            placeholder={view === 'sales' ? "$1000" : "Oct 30"} 
                        />
                    </div>
                    ...
                    {(view === 'sales' ? salesColumns : view === 'marketing' ? marketingColumns : techColumns).map((col) => (
                        <div key={col} className="flex-1 min-w-[200px] flex flex-col bg-gray-100/50 rounded-xl border border-gray-200">
                            ...
                            <div className={`p-3 border-b border-gray-200 flex items-center justify-between bg-gray-50 rounded-t-xl
                                ${view === 'marketing' ? 'border-t-4 border-t-pink-200' : ''}
                                ${view === 'tech' ? 'border-t-4 border-t-gray-300' : ''}
                                ${view === 'sales' ? 'border-t-4 border-t-green-200' : ''}
                            `}>
                                ...
                                <span className="bg-white text-gray-500 text-xs px-2 py-0.5 rounded-full border border-gray-200">
                                    {view === 'sales' 
                                        ...
                                        : view === 'marketing'
                                        ? marketingTasks.filter(t => t.status === col).length
                                ...
                                {view === 'sales' && deals.filter(d => d.status === col).map(deal => (
                                    <div key={deal.id} className="bg-white p-3 rounded-lg shadow-sm border border-gray-200 group hover:shadow-md transition-shadow relative">
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <button onClick={() => deleteItem(deal.id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            <select 
                                                className="w-full text-xs p-1 border border-gray-200 rounded bg-gray-50 text-gray-600 outline-none focus:border-[#15621B] text-black bg-white"
                                                value={deal.status} onChange={(e) => updateDealStatus(deal.id, e.target.value as DealStatus)}
                                            >
                                            ...
                                            {/* AUTOPILOT BUTTON */}
                                            <button 
                                                onClick={() => setAutopilotDeal(deal)}
                                                className="w-full flex items-center justify-center gap-1 text-xs bg-[#373737] text-white py-1.5 rounded hover:bg-black transition-colors"
                                            >
                                                <Zap size={12} className="text-[#FBEFD0]" />
                                                Manage / Autopilot
                                            </button>
                                        </div>
                                    </div>
                                ...
                                {view === 'marketing' && marketingTasks.filter(t => t.status === col).map(task => (
                                    <div key={task.id} className="bg-white p-3 rounded-lg shadow-sm border border-gray-200 group hover:shadow-md transition-shadow border-l-4 border-l-pink-500">
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <button onClick={() => deleteItem(task.id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                        
                                        <div className="flex items-center gap-2 mb-3">
                                            <span className={`text-xs px-2 py-1 rounded-md flex items-center gap-1 font-medium
                                                ${task.platform === 'Instagram' ? 'bg-pink-50 text-pink-700' : 'bg-blue-50 text-blue-700'}
                                            `}>
                                                {getPlatformIcon(task.platform)}
                                                {task.platform}
                                            </span>
                                            <span className="text-xs px-2 py-1 rounded-md bg-gray-100 text-gray-600 flex items-center gap-1">
                                                {getTypeIcon(task.contentType)}
                                                {task.contentType}
                                            </span>
                                        </div>
                                        ...
                                        <select 
                                            className="w-full text-xs p-1 border border-gray-200 rounded bg-gray-50 text-gray-600 outline-none focus:border-pink-500 text-black bg-white"
                                            value={task.status} onChange={(e) => updateMarketingStatus(task.id, e.target.value as MarketingStatus)}
                                        >
                                ...
                                {view === 'tech' && projects.filter(p => p.status === col).map(proj => (
                                    <div key={proj.id} className="bg-white p-3 rounded-lg shadow-sm border border-gray-200 group hover:shadow-md transition-shadow border-l-4 border-l-gray-700">
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <button onClick={() => deleteItem(proj.id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                        ...
                                        <div className="flex items-center justify-between text-xs font-medium text-gray-600 mb-3">
                                            <span className={`flex items-center gap-1 px-1.5 py-0.5 rounded ${proj.status === 'Deployed' ? 'bg-green-100 text-green-800' : 'bg-orange-50 text-orange-700'}`}>
                                                <Calendar size={10} /> {proj.deadline}
                                            </span>
                                        </div>

                                        <select 
                                            className="w-full text-xs p-1 border border-gray-200 rounded bg-gray-50 text-gray-600 outline-none focus:border-[#373737] text-black bg-white"
                                            value={proj.status} onChange={(e) => updateProjectStatus(proj.id, e.target.value as ProjectStatus)}
                                        >
